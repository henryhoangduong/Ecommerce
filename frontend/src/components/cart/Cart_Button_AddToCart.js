...
const Cart_Button_AddToCart = ({ productId = undefined,quantities=1 }) => {
  ...
  const { cartItems, setCartItems, handleUpdateQuantity, handleShow,findCartId } =
  ...
  const handleAddToCart = async () => {
    try {
      ...
      // cartItems not have item yet
        ...
        const response = await axios.post(url,{product_id:productId,quantities});
        ...
        const findCartItems_NameandPrice = (productId) => {
          const {name,price}=productsList.find((item)=>item.id==productId);
          return {name,price};
        }
        const {name,price} = findCartItems_NameandPrice(productId);
        const newCartItem = {...data,name,price}
        console.log("Cart_Button_AddToCart newCartItem :", newCartItem,"data: ",data);
